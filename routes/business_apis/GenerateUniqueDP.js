...
export default class GenerateUniqueDP extends BusinessQueryApiBase {
  ...
  static async mainProcess(req, res, next, user, transaction) {
    ...
    let DP = await DBTables.DP.findOne({ where: { id: DD_DW_DP.DPId }, transaction });
    if (!DP) {
      throw new Error('无灯片数据!');
    }

    if (!typeData.DWName) {
      let DW = await DBTables.DW.findOne({ where: { id: DD_DW_DPSnapshot.DWId }, transaction });
      if (!DW) {
    ...
    let GYS = await DBTables.GYS.findOne({ where: { id: DP.GYSId }, transaction });
    if (!GYS) {
      throw new Error('无生产供应商数据!');
    }
    typeData.GYSName = GYS.name;

    let PP = await DBTables.PP.findOne({ where: { id: DD_DW_DPSnapshot.PPId }, transaction });
    if (!PP) {
      throw new Error('无品牌数据!');
    }
    typeData.PPName = PP.name;

    let GT = await DBTables.GT.findOne({ where: { id: DD_DW_DPSnapshot.GTId }, transaction });
    if (!GT) {
