...
export default class ChuXiang extends BusinessApiBase {
  ...
  static async mainProcess(req, res, next, user, transaction) {
    ...
    for (let i = 0; i < HWEWMs.length; i++) {
      if (HWEWMs[i].type === DBTables.EWMType.WL) {
        // WL
        await this.chuXiangWYWL(HWEWMs[i], user, transaction);
      } else {
        // DP
        await this.chuXiangWYDP(HWEWMs[i], user, transaction);
      }
    }
  }

  static async chuXiangWYWL(WYWLEWM, user, transaction) {
    // 检查相关记录是否属于用户操作范围, 记录状态是否是可操作状态
  ...
  static async chuXiangWYDP(WYDPEWM, user, transaction) {
    // 检查相关记录是否属于用户操作范围, 记录状态是否是可操作状态
