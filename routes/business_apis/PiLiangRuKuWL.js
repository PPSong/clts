...
export default class PiLiangRuKuWL extends BusinessApiBase {
...
async function processWL(EWM, user, GYSId, transaction) {
  ...
  if (!tmpWYWL) {
    // 如果EWM不存在, 新建并直接入库
    await ppUtils.createWYWLAndRuKu(EWM, user, GYSId, transaction);
  } else {
    ...
    if (
      ...
      await ppUtils.changeWYWLStatusOnGYS(
        tmpWYWL,
        DBTables.WYWLStatus.RK,
        user,
        GYSId,
        transaction,
      );
    } else {
      throw new Error(`${tmpWYWL}的状态不能入库!`);
    }
  }
}
