...
export default class JieChuGuanLianKuaiDi extends BusinessApiBase {
  ...
  static async mainProcess(req, res, next, user, transaction) {
    ...
    // 检查KDXEWMs是属于FH状态
    await ppUtils.checkRecordsStatus(
      tmpKDXs,
    ...
    // 改变KDX中WL/DP的状态位ZX, 并添加WYWLCZ/WYDPCZ
    await ppUtils.changeWYWLsInKDXsStatus(KDXEWMs, DBTables.WYWLStatus.ZX, user, transaction);
    await ppUtils.changeWYDPsInKDXsStatus(KDXEWMs, DBTables.WYDPStatus.ZX, user, transaction);
    // end 改变KDX中WL/DP的状态位ZX, 并添加WYWLCZ/WYDPCZ
  }
}
